...
class ChurchServiceProcessor:
    ...
    async def get_latest_service(self, church_id: str) -> Optional[Dict]:
        """Fetch latest available service from kerkdienstgemist."""
    ...
    async def fetch_church_ids(self) -> List[Dict[str, str]]:
        ...
        try:
            response = self.supabase.table('churches')\
                .select('id, church_id, name, active, youtube_channel_name')\
                .eq('active', True)\
                .execute()
                ...
                return [
                    {
                        ...
                        'name': record['name'], 
                        'youtube_channel_name': record.get('youtube_channel_name')
                    } 
    ...
    async def send_report_to_subscribers(
        ...
        try:
            ...
            for subscriber in subscribers:
                ...
                # Check if the sermon has already been delivered
                if self.check_sermon_delivery(church_id, subscription_id, sermon_id):
    ...
    async def _process_media(
        ...
        if service_data.get('source') == 'youtube':
            # For YouTube, we will skip the standard download and directly use pytube
            temp_path = os.path.join(dir_info.path, "temp_download.mp4")

            if os.path.exists(temp_path):
                print(f"File already exists: {temp_path}")
                # If MP3 doesn't exist, convert it
                ...
                if os.path.exists(final_mp3_path):
                    ...
                    print("MP3 conversion needed but failed.")
                    return None
            else:
                print("Downloading YouTube video...")
                if not await self._download_youtube_video(download_url, temp_path):
                    return None
        ...
        # Kerkdienstgemist handling remains the same
    ...
    def _get_download_url(self, service_data: Dict) -> Tuple[Optional[str], Optional[str]]:
        ...
        if service_data.get('source') == 'youtube':
            # For YouTube, the 'download_url' is actually the watch_url
            return service_data.get('watch_url'), 'mp4'

        # Kerkdienstgemist fallback
        try:
    ...
    async def process_all_churches(self):
        ...
        for church in churches:
            church_uuid = church['id']
            church_id = church['church_id']
            church_name = church['name']
            youtube_channel_name = church.get('youtube_channel_name')

            print(f"\nProcessing church: {church_name} (ID: {church_id})")
            
            if youtube_channel_name and youtube_channel_name.strip():
                service_info = await self.get_latest_youtube_service(youtube_channel_name.strip())
            else:
...
def clean_html_report(file_path):
    """
    Cleans the HTML report file by removing ```html at the start, the closing ```,
    and everything after the closing </html> tag.
    """
    try:
...
def load_html_report_prompt():
    try:
        with open('html_report_prompt.txt', 'r', encoding='utf-8') as f:
            return f.read()
    except Exception as e:
        print(f"Error loading HTML report prompt: {e}")
        return None
